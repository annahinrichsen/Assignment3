...
Model_pause = lmerTest::lmer(npause ~ Diagnosis * trial + (1+trial|Subject), data = data_all)

Model_dur = lmerTest::lmer(dur..s. ~ Diagnosis * trial + (1+trial|Subject), data = data_all)

Model_range = lmer(PR ~ Diagnosis + (1|Study)) #we can also use study as a random effect - cause researchers that run experiments do it diffenretly 
